...
} from 'src/entities/subscription-package.entity';
...
export class SubscriptionService {
  ...
  async getUniquePeriodTypes() {
    return this.subscriptionPackageRepository
  ...
  async getSubscriptionPackagesGroupedByName() {
    const packages = (await this.subscriptionPackageRepository
      .createQueryBuilder('package')
      .select(
        `
  ...
  async getSubscriptionPackagesByName(name: string) {
    return this.subscriptionPackageRepository
      .createQueryBuilder('package')
      .leftJoinAndSelect(
        'package.subscriptionPackageFeatures',
  ...
  async getSubscriptionPackagesByPeriodType(
  ...
  async getUnactiveSubscriptionsWithFeatures() {
    ...
    return this.userSubscriptionRepository
      ...
      .leftJoinAndSelect(
        'subscriptionPackage.subscriptionPackageFeatures',
        'subscriptionPackageFeatures',
      )
      .leftJoinAndSelect('subscriptionPackageFeatures.feature', 'feature')
  ...
  async getUserSubscriptionPackageFeatures(userId: number) {
    ...
    const subscriptionPackageFeatures =
      await this.subscriptionPackageFeatureRepository
        .createQueryBuilder('subscriptionPackageFeature')
        .leftJoinAndSelect(
          'subscriptionPackageFeature.subscriptionPackage',
          'subscriptionPackage',
        )
        .leftJoinAndSelect('subscriptionPackageFeature.feature', 'feature')
        .where('subscriptionPackage.id = :subscriptionPackageId', {
          subscriptionPackageId: userSubscription.subscriptionPackageId,
        })
        .getMany();

    return subscriptionPackageFeatures.map(
  ...
  async createUserSubscription(
    ...
    const subscriptionPackage = await this.subscriptionPackageRepository
      .createQueryBuilder('package')
      .leftJoinAndSelect(
        'package.subscriptionPackageFeatures',
        'subscriptionPackageFeatures',
    ...
    const endDate = DateTime.now()
      .plus({ [subscriptionPackage.periodType]: 1 })
      .toJSDate();
    ...
    await manager.transaction(async (manager) => {
      ...
      const features = subscriptionPackage.subscriptionPackageFeatures.map(
        (feature) => feature.feature,
      );

      await this.featuresService.grantAccessToFeatures(
        userId,
  ...
  async deleteSubscriptionAcesses(
    ...
    await manager.transaction(async (manager) => {
      ...
      const features = await this.getUserSubscriptionPackageFeatures(
        userSubscription.userId,
      );

      await this.featuresService.deleteAccessToFeatures(
        userSubscription.userId,
        features.map((feature) => feature.id),
        manager,
      );
      ...
      await this.notificationService.sendNotification(
        user,
        `Your subscription has been canceled. Access to features ${featureNames} has been revoked`,
      );
    });
  }
  ...
  async cancelSubscription(
    ...
    const userSubscription =
      await this.findActiveUserSubscription(userSubscriptionId);
