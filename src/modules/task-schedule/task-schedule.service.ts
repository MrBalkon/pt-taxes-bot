...
import {
  TaskShedule,
  TaskSheduleType,
} from 'src/entities/task-schedule.entity';
...
export class TaskSheduleService implements OnModuleInit {
  ...
  async handleOneShotTasks() {
    ...
    const taskShedules =
      await this.taskSheduleRepository.getOneShotTaskShedules();
  ...
  async sheduleCronJobs() {
    ...
    const taskShedules =
      await this.taskSheduleRepository.getRecurringTaskShedules();
    const sheduledTasks = [];
  ...
  private async addTaskToQueueBulk(taskShedules: TaskShedule[]) {
    const tasksPayloads = taskShedules.map((taskShedule) =>
      this.prepareTaskPayload(taskShedule),
    );
    try {
  ...
  async createOneShotTaskShedule(
    taskId: number,
    taskPayload: any,
    oneShotDate: Date,
  ) {
  ...
  async getUserOneShotTaskHigherThanDate(
    taskId: number,
    userId: number,
    date: DateTime,
  ) {
    return this.taskSheduleRepository.getOneShotTaskByTaskAndUserId(
      taskId,
      userId,
      date,
    );
