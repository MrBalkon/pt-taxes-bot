...
export class FeatureService {
  ...
  async grantAcessToFeature(userId: number, featureId: number) {
    const existingAccess = await this.featureAccessRepository.findOneBy({
      userId,
      featureId,
    });
    if (existingAccess) {
    ...
    await this.notificaitonService.sendNotification(
      user,
      `You have been granted access to feature ${feature.name}. Please, fill needed information`,
      {
  ...
  async grantAccessToFeatures(
    ...
    await manager.save(
      FeatureAccess,
      featureIds.map((featureId) => ({ userId, featureId })),
    );
  }

  async deleteAccessToFeatures(
    userId: number,
    featureIds: number[],
    manager: EntityManager = this.connection.manager,
  ) {
