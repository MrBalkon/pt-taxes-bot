...
import { OperationErrorType, OperationStatus } from 'src/entities/operation.entity';
import { TaskProcessingQueueService } from '../task-processing-queue/task-processing-queue.service';
import { ServiceUnavailableError, TaskInputFieldsException, WrongCredentialsError } from '../task-processing-queue/task-processing-queue.error';
...
import { TaskRetryPolicy, TaskRetryPolicyType } from 'src/entities/task-retry-policy.entity';
...
export class TaskProcessingQueueConsumer {
	...
	private async handleError(task: Job<TaskProcessingPayload>, payload: TaskProcessingPayload, done: DoneCallback, e: Error) {
		const operationErrorType = this.getOperationErrorType(e);
		await this.operationService.updateOperationStatus(payload.taskUid, OperationStatus.FAIL, e.message, operationErrorType);
	...
	private async handleTaskRetryPolicy(task: Job<TaskProcessingPayload>, payload: TaskProcessingPayload, errorType: OperationErrorType) {
		...
		this.logger.log(`[${task.data.taskUid}] Applying for task: ${retryPolicy.retryPolicy} with value = ${retryPolicy.value}`);

		if (payload?.userId) {
			const tasksInFuture = await this.taskSheduleService.getUserOneShotTaskHigherThanDate(dbTask.id, payload?.userId, DateTime.now());

			if (tasksInFuture?.length) {
				this.logger.log(`[${task.data.taskUid}] Task already has retry in future`);
				return;
			}
		}
	...
	private async applyRetryPolicy(dbTask: Task,task: Job<TaskProcessingPayload>, retryPolicy: TaskRetryPolicy): Promise<string | null> {
		let message = `🚨 We were unable to process automatic task "${dbTask.description} 🚨".\n\n`;
			...
			case TaskRetryPolicyType.RETRY_AFTER_DAYS:
				{
					...
					await this.taskSheduleService.createOneShotTaskShedule(dbTask.id, task.data, retryDate.toJSDate());
