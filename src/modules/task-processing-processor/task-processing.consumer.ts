...
import { TaskRetryPolicyType } from 'src/entities/task-retry-policy.entity';
...
export class TaskProcessingQueueConsumer {
	...
	private async handleError(task: Job<TaskProcessingPayload>, payload: TaskProcessingPayload, done: DoneCallback, e: Error) {
		...
		await this.handleTaskRetryPolicy(task, operationErrorType);
	...
	private async handleTaskRetryPolicy(task: Job<TaskProcessingPayload>, errorType: OperationErrorType) {
		...
		this.logger.log(`[${task.data.taskUid}] Applying for task: ${retryPolicy.retryPolicy}`);

		switch (retryPolicy.retryPolicy) {
			case TaskRetryPolicyType.RETRY_AFTER_DAYS:
				{
					const daysValue = Number(retryPolicy.value);
					const retryDate = DateTime.now().plus({ days: daysValue }).toJSDate();
					await this.taskSheduleService.createOneShotTaskShedule(dbTask.id, task.data, retryDate);
					break;
				}
			case TaskRetryPolicyType.RETRY_AFTER_HOURS:
				{
					const hoursValue = Number(retryPolicy.value);
					const retryDate = DateTime.now().plus({ hours: hoursValue }).toJSDate();
					await this.taskSheduleService.createOneShotTaskShedule(dbTask.id, task.data, retryDate);
					break;
				}
			case TaskRetryPolicyType.RETRY_AFTER_WEEKS:
				{
					...
					const retryDate = DateTime.now().plus({ weeks: weeksValue }).toJSDate();
					await this.taskSheduleService.createOneShotTaskShedule(dbTask.id, task.data, retryDate);
					break;
				}
