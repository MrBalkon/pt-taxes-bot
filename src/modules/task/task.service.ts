...
export class TaskService implements OnModuleInit {
  ...
  async getTaskById(
    ...
    return manager.getRepository(Task).findOneBy({ id: taskId });
  }

  async getFullTasksByUserId(
    userId: number,
    manager: EntityManager = this.connection.manager,
  ) {
    return manager
  ...
  async getTaskStepsBySystemName(
    ...
    return this.executionStepRepository.getExecutionStepsByTaskSystemName(
      taskSystemName,
      manager,
    );
  }

  searchForTasksWithOutputFields(
    userAllowedTaskIds: number[],
    outputFieldIds: number[],
  ) {
    ...
    return Object.values(this.tasksNameMap).filter((task) => {
      ...
      return task.outputFields.some((taskOutputField) =>
        outputFieldIds.includes(taskOutputField.fieldId),
      );
    });
  }

  private async updateLocalTasksMap(
    manager: EntityManager = this.connection.manager,
  ) {
  ...
  async getSystemMissedTasks() {
    ...
    const recurringSystemTaskShedules =
      await this.taskSheduleRepository.getRecurringSystemTasks();

    const systemTasksIds = recurringSystemTaskShedules.map(
      (task) => (task.taskPayload as any)?.data?.splitTaskId,
    );

    const taskSheduleMap = recurringSystemTaskShedules.reduce(
      (acc, taskShedule) => {
    ...
    const operations =
      await this.operationsService.getSystemOperationsByTaskIds(systemTasksIds);
    ...
    const missedOperations = operations.filter((operation) => {
      ...
      switch (taskShedule.periodTime) {
        case CronPeriod.MONTHLY:
          return (
            lastExecutionTime.month === now.month && now.day > taskShedule.day
          );
