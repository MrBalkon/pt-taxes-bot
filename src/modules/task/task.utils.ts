...
const TIME_RANGE_COMPARE_MAP = Object.entries(TIME_RANGE_COMPARE).reduce((acc, [key, value]) => {
	value.forEach((val) => {
		if (!acc[key]) {
			acc[key] = {};
		}
...
export const compareTaskFields = (originalField: UserField, fieldA: TaskFieldsSearch, fieldB: TaskOutputField) => {
	...
	const timeRangeTypeACompare = TIME_RANGE_COMPARE_MAP?.[timeRangeTypeA]?.[timeRangeTypeB];
