...
export class TaskProcessingQueueService {
  ...
  async addJobByTelegramId<T>(
    telegramId: number,
    payload: TaskProcessingPayloadCall<T>,
  ) {
    const user = await this.userServices.getUserByTelegramId(
      String(telegramId),
    );
  ...
  async addQueueJobBulk<T>(tasksPayloads: TaskProcessingPayloadCall<T>[]) {
    ...
    await this.operationService.createOperationsBulk(
      tasks.map((task) => ({
  ...
  async addQueueJob<T>(jobData: TaskProcessingPayloadCall<T>) {
    ...
    this.logger.log(`[${uuid}] Received task: ${jobData.type}`);

    const job = await this.queue.getJob(uuid);

    if (!job) {
      this.logger.log(`[${uuid}] No existing job, creating new one`);
