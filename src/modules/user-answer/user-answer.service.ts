...
export class UserAnswerService {
  ...
  async getUserMetaFieldsByIds(userId: number, fieldIds: number[]) {
    ...
    return this.userAnswerSerializer.serilizeFields(
      answers,
      'field.systemName',
    );
  }
  ...
  async getUserAllMetaFieldsByTaskIds(userId: number, inputTaskIds: number[]) {
    ...
    const answers = await this.answerRepository.getAnswersByInputTaskIds(
      userId,
      inputTaskIds,
    );
  ...
  async getUserAllMetaFieldsSystemNameByTaskIds(
    userId: number,
    inputTaskIds: number[],
  ) {
    if (!inputTaskIds.length) {
  ...
  async saveAnswersBulkByFieldSystemName(
    userId: number,
    data: CreateOrUdpdateFieldAnswer[],
  ) {
    const answers = data.map((item) => {
      return {
        userId,
        fieldId: this.userFieldService.getUserFieldByName(item.fieldSystemName)
          .id,
  ...
  async answersSyncByFieldSystemName(
    userId: number,
    ...
    const answersToDelete = existingAnswers.filter((existingAnswer) => {
      return !data.find((newAnswer) =>
        isEqual(newAnswer.fieldValue, existingAnswer.fieldValue),
      );
    });

    // find answers that are not in the old list
    const answersToCreate = data.filter((newAnswer) => {
      return !existingAnswers.find((existingAnswer) =>
        isEqual(newAnswer.fieldValue, existingAnswer.fieldValue),
      );
    });
