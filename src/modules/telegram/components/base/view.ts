...
Handlebars.registerHelper('singleLineOnly', function (options) {
  // "this" cannot be provided to inline function!
  return options.fn(this).replace(/[\r\n]+/gm, '');
});
Handlebars.registerHelper('singleSpaceOnly', function (options) {
  // "this" cannot be provided to inline function!
  return options.fn(this).replace(/\s\s+/g, ' ');
});

// register helper that gonna replace all left spaces
Handlebars.registerHelper('leftTrim', function (options) {
  // "this" cannot be provided to inline function!
  return options.fn(this).replace(/^\s+/, '');
});

// register helper that remove all tabs
Handlebars.registerHelper('removeTabs', function (options) {
  // "this" cannot be provided to inline function!
