...
export class TaskManagerService implements Task {
  ...
  async run(taskPayload: TaskProcessingPayload): Promise<void> {
    ...
    if (!userdbDasks.length) {
      this.logger.log(
        `[${taskPayload.taskUid}] No tasks found for user ${user.id}`,
      );
      return;
    }
  ...
  private async validateParentOperaration(
    taskPayload: TaskProcessingPayload,
  ): Promise<boolean> {
    if (!taskPayload?.parentOperationId) {
      return true;
    }

    const parentOperation = await this.operationsService.getOperationById(
      taskPayload.parentOperationId,
    );
  ...
  async getNextTasksToExecuteV2(user: User, tasks: DBTask[]) {
    ...
    const tasksToProcess = tasks.reduce<DBTask[]>((acc, task) => {
      task.taskFields = task.taskFields.filter(
        (field) =>
          !taskFieldParser.userHasField(field, metaFields[field.fieldId]),
      );
      acc.push(task);
    ...
    const tasksTree = tasks.reduce<Record<number, any>>((acc, task) => {
      ...
      tasks.forEach((extractionTask) => {
        const hasMatch = task.taskFields.some((inputField) =>
          extractionTask.outputFields.some(
            (taskField) => taskField.fieldId === inputField.fieldId,
          ),
        );
