...
export class DynamicTask implements Task {
  ...
  async run(task: TaskProcessingPayload): Promise<void> {
    ...
    await this.telegramService.sendMessage(
      user.telegramId,
      'Finished with login to Seg Social!',
    );
  }
  ...
  async runInSelenium(
    ...
    const scenarioTasks =
      await this.executionScenarioService.getExecutionMapTreeByTaskSystemName(
        task.type,
      );
    const commands =
      await this.executionCommandService.getExecutionCommandsByTaskSystemName(
        task.type,
      );
