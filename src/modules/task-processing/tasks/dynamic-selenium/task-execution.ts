...
export class TaskExecution {
	...
	async executeStep(step: ExecutionStep): Promise<void> {
		try {
			...
			const result = await executionSource[commandMethod](...executionArguments)

			if (step.executionArguments?.resultNamePath) {
				this.executionContext.set(step.executionArguments.resultNamePath, result)
			}
		} catch (e) {
	...
	private getExecutionSource(step: ExecutionStep, command: ExecutionCommand): any {
		if (step.executionArguments?.executionSource) {
			const source = this.executionContext.get(step.executionArguments.executionSource)

			if (!source) {
				throw new Error(`Execution source ${step.executionArguments.executionSource} not found`)
			}
		...
		if (command.executionSource) {
			...
			if (!source) {
				throw new Error(`Execution source ${command.executionSource} not found`)
			}
	...
	private applyExecutionPath(executionPath: ExecutionPath[], value: any): any {
		for (const path of executionPath) {
			if (path?.executionMethod) {
				value = this.executionContext.get(path.executionSource)[path.executionMethod](value)
			}
		}
		return value
	}
