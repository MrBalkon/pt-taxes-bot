import {
  ...
  ScenarionChildCondition,
} from 'src/entities/execution-scenario-task.entity';
import {
  StepExecutionError,
  TaskExecution,
  TaskExecutionArgs,
...
export class ScenarioTreeExecution extends TaskExecution {
  ...
  async executeTask(): Promise<void> {
    while (this.currentScenarioTask) {
      ...
      this.logger.log(
        `Executing scenario task: ${this.currentScenarioTask.id}`,
      );
  ...
  private async executeScenario(currentScenario: ExecutionScenario) {
    const steps = currentScenario.scenarioSteps.map(
      (scenarioStep) => scenarioStep.executionStep,
    );
    ...
    try {
      ...
      if (currentScenario?.errorProduce) {
        this.executionContext.set(
          `exceptions.${currentScenario.errorProduce}`,
          true,
        );
        return;
      }
  ...
  private init(scenarioTasks: ExecutionScenarionTask[]) {
    this.logger.log(
      `Init scenario tree execution with ${scenarioTasks.length} tasks`,
    );
  ...
  private getNextScenarioTask() {
    ...
    const scenarioTasksCandidates =
      this.scenarioTasksMap[currentScenarioTask.id];
    if (!scenarioTasksCandidates) {
      return null;
    }

    const scenarioTask = this.chooseScenarionBasedOnConditions(
      scenarioTasksCandidates,
    );

    return scenarioTask;
  }

  private chooseScenarionBasedOnConditions(
    scenarioTasks: ExecutionScenarionTask[],
  ): ExecutionScenarionTask | null {
