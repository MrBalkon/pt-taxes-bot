...
import {
  goFaturaVerdePage,
  retrieveFaturaVerdeIncome,
} from '../../selenium-scenarios/financais/retrieve-fatura-verde.scenario';
import {
  DownloadFinancaisFaturaTable,
  DownloadFinancaisFaturaTableCSituaca,
  DownloadFinancaisFaturaTableTipoRecibo,
} from '../../selenium-scenarios/financais/download-financais';
...
export class FinancaisFillData implements Task {
  ...
  async runInSelenium(
    ...
    try {
      ...
      await this.userAnswerService.saveAnswersBulkByFieldSystemName(
        user.id,
        answers,
      );
    } catch (e) {
      await this.notificaitonService.sendNotification(
        user,
        'Something went wrong with your declaration',
      );
      throw e;
    }
  }
