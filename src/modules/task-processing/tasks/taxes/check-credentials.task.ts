...
export class CheckCredentialsTask implements Task {
  ...
  async run(task: TaskProcessingPayload): Promise<void> {
    ...
    return this.seleniumService.execute(async (driver) =>
      this.runInSelenium(driver, user, metaFields),
    );
  }
  ...
  async runInSelenium(
    ...
    try {
      ...
      if (metaFields.niss && metaFields.segSocialPassword) {
        ...
        const segSocialFields = await this.checkSegSocialCredentialsErrors(
          user,
          metaFields,
          driver,
        );
        ...
        if (!segSocialFields.length) {
          await this.questionService.saveAnswerByFieldSystemName(
            user.id,
            'hasUserCheckedSegSocialCreds',
            { fieldValue: 'yes' },
          );
        } else {
          await this.questionService.deleteAnswer(
            user.id,
            'hasUserCheckedSegSocialCreds',
          );
        }
      ...
      if (metaFields.nif && metaFields.financasPassword) {
        ...
        const finacasFields = await this.checkFinancasCredentialsErrors(
          user,
          metaFields,
          driver,
        );
      ...
      if (incorrectFields.length) {
        const fieldSystemNames = incorrectFields.map(
          (field) => field.field.systemName,
        );
        const fieldNames = incorrectFields.map(
  ...
  async checkSegSocialCredentialsErrors(
    user: User,
    metaFields: Record<string, any>,
    driver: WebDriver,
  ): Promise<string[]> {
    ...
    try {
      await this.notificaitonService.sendNotification(
        user,
        "We're trying to login to Seg Social with your credentials...",
      );
  ...
  async checkFinancasCredentialsErrors(
    user: User,
    metaFields: Record<string, any>,
    driver: WebDriver,
  ): Promise<string[]> {
    ...
    try {
      await this.notificaitonService.sendNotification(
        user,
        "We're trying to login to Finanças with your credentials...",
      );
