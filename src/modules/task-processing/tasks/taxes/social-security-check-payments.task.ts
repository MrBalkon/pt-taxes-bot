...
import {
  ...
  goPayementsPage,
  retrievePayments,
} from '../../selenium-scenarios/seg-social/payment/retrieve-payments.scenario';
...
export class SocialSecurityCheckPayments implements Task {
  ...
  async run(task: TaskProcessingPayload): Promise<void> {
    await this.seleniumService.execute(async (driver) =>
      this.runInSelenium(driver, task),
    );
  }
  ...
  async runInSelenium(
    ...
    const { answersToDelete, answersToCreate } =
      await this.answersService.answersSyncByFieldSystemName(
        task.user.id,
        'taxPayements',
        dbPayments,
