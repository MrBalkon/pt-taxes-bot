...
const goRegisterDeclarationPage = goPage(
  'Registar Declaração',
  'https://app.seg-social.pt/ptss/qlf/trabalhadores-independentes/registar_declaracao?frawmenu=1&dswid=-1',
);
...
export const fillDeclarationWaitAndPressNext = async (driver: WebDriver) => {
  await driver.wait(
    until.elementLocated(By.name('mainForm:proximoPassoBtn')),
    400,
  );
  const nextStepButton = await driver.findElement(
    By.name('mainForm:proximoPassoBtn'),
  );
  await nextStepButton.click();
};

export const fillTrimesterIncome = async (
  driver: WebDriver,
  income: string,
  month: number,
) => {
  await driver.wait(
    until.elementLocated(
      By.xpath(
        `//*[@id="mainForm:rendimentosPanel:ps:valorMes${month}_input"]`,
      ),
    ),
    300,
  );
  const trimesterInput = await driver.findElement(
    By.xpath(`//*[@id="mainForm:rendimentosPanel:ps:valorMes${month}_input"]`),
  );
  await trimesterInput.sendKeys(income);
};

export const checkIfDeclarationIsFilled = async (
  driver: WebDriver,
...
export const socialSecurityFillTrimestralDeclaration = async (
  driver: WebDriver,
  data: SocialSecurityDeclarationData,
) => {
  ...
  const radioYesInput = await driver.findElement(
    By.xpath(
      '//*[@id="mainForm:temRendimentosCustomRadio:customRadioPanel"]/div[1]/label',
    ),
  );
  ...
  const servicesProvisionElement = await driver.findElement(
    By.xpath('//*[@id="mainForm:rendimentosPanel"]/h3[1]/div/div/font/font'),
  );
  ...
  const radioYesInput2 = await driver.findElement(
    By.xpath(
      '//*[@id="mainForm:temRendimentosCustomRadio:customRadioPanel"]/div[1]/label',
    ),
  );
