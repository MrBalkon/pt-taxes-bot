...
export const financaisLogin = async (driver: WebDriver, { nif, password }) => {
  try {
    const nifTab = await driver.findElement(
      By.xpath("//span[contains(text(), 'NIF')]"),
    );
    await nifTab.click();
...
export const gotToAtividadePage = goPage(
  'Atividade',
  'https://sitfiscal.portaldasfinancas.gov.pt/integrada/presentation?queryStringS=targetScreen%3DecraActividade%26hmac%3DNYiJFiu6j3kZKB2L7YDSHFj8GPM%3D',
);
...
export const retrieveCategoryData = async (driver: WebDriver) => {
  const workType = await driver.findElement(
    By.xpath(
      "//dt[contains(text(), 'Tipo de Sujeito Passivo')]//following-sibling::dd",
    ),
  );
...
export const getActivityStartDate = async (driver: WebDriver) => {
  const startDate = await driver.findElement(
    By.xpath("//dt[contains(text(), 'Data de Início')]//following-sibling::dd"),
  );
