...
export class TaskSheduleRepository extends Repository<TaskShedule> {
  ...
  async getOneShotTaskByTaskAndUserId(
    taskId: number,
    userId: number,
    date: DateTime,
  ) {
    return this.createQueryBuilder('ts')
      ...
      .andWhere(
        "COALESCE(ts.task_payload::jsonb->>'userId', NULL)::int = :userId",
        { userId },
      )
  ...
  async getRecurringSystemTasks() {
    return this.createQueryBuilder('ts')
      ...
      .andWhere('ts.isActive = true')
      .andWhere(
        "COALESCE(ts.task_payload::jsonb->'data'->'splitTaskId', NULL) IS NOT NULL",
      )
      .getMany();
  }
}
