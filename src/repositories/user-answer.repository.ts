...
export class UserAnswerRepository extends Repository<UserAnswer> {
  ...
  async getAnswersByUserIdAndTaskSystemName(
    userId: number,
    systemName: string,
  ) {
    return this.defaultSelector()
  ...
  async getAnswersByUserIdAndFieldSystemNames(
    userId: number,
    fieldsSystemNames: string[],
  ) {
    return this.defaultSelector()
      .where('answer.userId = :userId', { userId })
      .andWhere('field.system_name IN (:...fieldsSystemNames)', {
        fieldsSystemNames,
      })
      .getMany();
  }
  ...
  async deleteAnswer(userId: number, fieldsSystemName: string) {
    ...
    return this.delete(answers[0].id);
  }

  async deleteAnswerBulk(userId: number, fieldsSystemNames: string[]) {
    const answers = await this.defaultSelector()
      .where('answer.userId = :userId', { userId })
      .andWhere('field.systemName IN (:...fieldsSystemNames)', {
        fieldsSystemNames,
      })
      .getMany();
    ...
    return this.delete(answers.map((answer) => answer.id));
  }

  private prepareAnswerSaveData(
    data: DeepPartial<UserAnswer>,
    uniqueString?: string,
  ) {
  ...
  private defaultSelector() {
    return this.createQueryBuilder('answer').leftJoinAndSelect(
      'answer.field',
      'field',
    );
    //   .select(
