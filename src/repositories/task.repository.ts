...
export class TaskRepository extends Repository<Task> {
	...
	async getTasksByUserId(userId: number, manager: EntityManager = this.manager) {
	...
	async getTaskStepsBySystemName(taskSystemName: string, manager: EntityManager = this.manager) {
		return manager.getRepository(Task)
			.createQueryBuilder('task')
			.leftJoinAndSelect('task.executionScenarios', 'executionScenarios')
			.leftJoinAndSelect('executionScenarios.executionScenario', 'executionScenario')
